...
const bookingAvailabilityController=async(req,res)=>{
  try {
    ...
    const appointments = await appointmentModel.find({
      ...
      date,
      time: {
        $gte: fromTime,       //gte matlab greater than equal to
        $lte: toTime,         //lte matlab lesser than equal to
      },
    });
...
const getUserProfileInfoController=async(req,res)=>{
  try {
      ...
      res.status(200).send({
          success: true,
          message: "User profile data fetch success",
          data: user,
        });
  } catch (error) {
      console.log(error)
      res.status(500).send({
          success:false,
          message:'Error in fetching User Profile Details',
          error
      })
  }
